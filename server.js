...
// ── Lightweight ping endpoint for health check + cronjob.com keep-alive ──
app.get('/api/ping', (req, res) => {
  res.json({ success: true, pong: true, status: 'ok', uptime: process.uptime(), timestamp: new Date().toISOString() });
});
...
app.get('*', (req, res) => {
  ...
  if (!fs.existsSync(indexPath)) {
    return res.status(500).send('<h1>Gold_Crew OSINT</h1><p>index.html introuvable. Vérifiez que tous les fichiers sont dans le dossier du projet sur GitHub.</p><p>Chemin attendu: ' + indexPath + '</p>');
  }
  res.sendFile(indexPath, (err) => {
    if (err) {
      console.error('[GC] Erreur envoi index.html:', err.message);
      res.status(500).send('<h1>Gold_Crew OSINT</h1><p>Erreur serveur: ' + err.message + '</p>');
    }
  });
});
