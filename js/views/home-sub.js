...
const HomeSubView = (() => {
  ...
  async function render(container) {
    ...
    // Load announcements (route utilisateur — la route admin nécessite un token admin
    // et échouait silencieusement pour les comptes normaux, qui ne voyaient jamais
    // les vraies annonces envoyées par le serveur/Supabase).
    let announcements = [];
    try {
      const r = await GCApi.getAnnouncements();
      if (r.success) announcements = r.announcements || [];
    } catch {}
