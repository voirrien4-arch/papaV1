...
window.GCDashboardView = DashboardView;
  ...
  function navigateSubView(subId) {
    ...
    document.getElementById('page-title').textContent = t(titles[subId] || subId);
    document.querySelectorAll('[data-nav]').forEach(b => b.classList.toggle('active', b.dataset.nav === subId));
    const viewMap = { home: HomeSubView, search: SearchSubView, history: HistorySubView, promo: PromoSubView, favorites: FavoritesSubView, people: PeopleSubView, import: ImportSubView, stats: StatsSubView, profile: ProfileSubView, settings: SettingsSubView };
    const view = viewMap[subId];
    if (view) view.render(document.getElementById('view-container'));
  }
