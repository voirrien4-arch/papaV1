...
const DashboardView = (() => {
        ...
        <div class="flex-1 flex flex-col min-w-0">
          ...
          <header class="sticky top-0 z-40 h-12 flex items-center justify-between px-4 glass-card" style="border-radius:0;border-top:none;border-left:none;border-right:none">
            ...
            <div class="flex items-center gap-2">
              ...
              <div id="header-avatar" class="w-7 h-7 rounded flex items-center justify-center text-xs font-bold overflow-hidden" style="background:rgba(0,255,65,0.1);color:var(--green);border:1px solid rgba(0,255,65,0.2);font-family:var(--font-mono)">
                ${user.avatar ? `<img src="${user.avatar}" alt="" class="w-full h-full object-cover" />` : (user.firstName?.[0] || user.username[0]).toUpperCase()}
              </div>
            </div>
          </header>
  ...
  return { render, navigateSubView, updateNotifDot, refreshCrewDisplay, refreshHeaderAvatar };
})();
